      ...
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-10 xs:py-12 sm:py-14 md:py-16">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-8 xs:mb-10">
            <span className="inline-block text-xs xs:text-sm font-semibold px-3 py-1 rounded-full mb-3 bg-emerald-100 text-emerald-700">Youth Ventures</span>
          ...
          <div className="mb-6 xs:mb-8">
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              className="w-full max-w-md mx-auto block px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 text-sm mb-4"
              placeholder="Search products or vendors..." />
            <div className="flex flex-wrap gap-2 justify-center">
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${activeCategory === cat ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-emerald-400'}`}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-2xl border border-gray-200">
              ...
              <p className="text-gray-500 text-sm mb-5">Be the first to list your product on the SWK Marketplace!</p>
                ...
                <div key={product.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${categoryColors[product.category] || 'bg-gray-100 text-gray-600'}`}>
                        {product.category}
                      </span>
                    ...
                    <p className="text-xs text-gray-500 mb-1">by <span className="font-medium text-emerald-600">{product.business}</span></p>
                    <p className="text-xs text-gray-600 leading-relaxed flex-1 mb-3 line-clamp-3">{product.description}</p>
          ...
          {/* Info banner */}
          <div className="mt-10 bg-emerald-700 rounded-2xl p-6 xs:p-8 text-white text-center">
            <h2 className="text-xl xs:text-2xl font-bold mb-2">Are you a young entrepreneur?</h2>
            <p className="text-emerald-100 text-sm xs:text-base mb-5 max-w-xl mx-auto">
            ...
            <button onClick={() => setVendorModal(true)} className="bg-white text-emerald-700 font-semibold px-6 py-2.5 rounded-xl hover:bg-gray-100 transition-colors text-sm xs:text-base">
              List Your Product →
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 xs:p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-auto p-5 xs:p-6 sm:p-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button aria-label="Close" className="text-gray-500 hover:text-gray-700 text-xl" onClick={() => setOrderModal(null)}>✕</button>
            </div>
            ...
            <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-xl p-3 mb-5">
              {orderModal.imageUrl ? (
                ...
                <div className="w-14 h-14 bg-emerald-200 rounded-lg flex items-center justify-center flex-shrink-0"><span className="text-2xl">🌱</span></div>
              )}
              <div>
                <p className="font-semibold text-gray-900 text-sm">{orderModal.productName}</p>
                <p className="text-xs text-gray-500">by {orderModal.business}</p>
                <p className="text-sm font-bold text-emerald-700">{orderModal.price ? `GHS ${orderModal.price}` : 'Contact for price'}{orderModal.unit ? ` / ${orderModal.unit}` : ''}</p>
              </div>
            </div>
            ...
            {orderStatus === 'success' ? (
              <div className="flex flex-col items-center py-8 text-center">
                ...
                <p className="text-sm text-gray-600 mb-5">Thank you, {order.buyerName.split(' ')[0]}! We'll contact you shortly to confirm your order and arrange delivery.</p>
              ...
              <form className="space-y-4" onSubmit={handleOrderSubmit}>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="text" value={order.buyerName} onChange={e => setOrder({ ...order, buyerName: e.target.value })}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="Your name" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input required type="email" value={order.buyerEmail} onChange={e => setOrder({ ...order, buyerEmail: e.target.value })}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="your.email@example.com" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <textarea required rows={2} value={order.deliveryAddress} onChange={e => setOrder({ ...order, deliveryAddress: e.target.value })}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="Where should we deliver?" />
                </div>
                <div>
                  ...
                  <textarea rows={2} value={order.notes} onChange={e => setOrder({ ...order, notes: e.target.value })}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="Any special requests..." />
                </div>
                {orderStatus === 'error' && <p className="text-xs text-red-500">Something went wrong. Please try again.</p>}
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setOrderModal(null)} className="px-4 py-2 border rounded-lg text-gray-600 text-sm">Cancel</button>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-xl mx-auto p-5 xs:p-6 sm:p-8 max-h-[90vh] overflow-y-auto">
            ...
            {vendorStatus === 'success' ? (
              <div className="flex flex-col items-center py-8 text-center">
                ...
                <p className="text-sm text-gray-600 mb-5">Thank you! We'll review your product and publish it within 24–48 hours. We'll notify you at {vendor.email}.</p>
              ...
              <form className="space-y-4" onSubmit={handleVendorSubmit}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="text" value={vendor.name} onChange={e => setVendor({ ...vendor, name: e.target.value })}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="Your full name" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                    <input required type="email" value={vendor.email} onChange={e => setVendor({ ...vendor, email: e.target.value })}
                      className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="you@email.com" />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                ...
                <div>
                  ...
                  <input required type="text" value={vendor.location} onChange={e => setVendor({ ...vendor, location: e.target.value })}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="e.g. Accra, Kumasi..." />
                </div>

                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide pt-2">Product Details</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                ...
                <div>
                  ...
                  <input type="url" value={vendor.imageUrl} onChange={e => setVendor({ ...vendor, imageUrl: e.target.value })}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm" placeholder="https://..." />
